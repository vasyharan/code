...
use sumtree::{Node, Tree as SumTree};
...
impl Rope {
    ...
    pub fn insert(&self, offset: usize, slab: Slab) -> Result<Self> {
        if offset > self.len() {
        ...
        let root = Arc::new(insert_recursive(&self.0, offset, slab));
        todo!()
        // let root = Arc::new(insert(&self.0, offset, slab));
        // let root = make_black(root);
        // Ok(Rope(root))
    }
}

fn insert_recursive(node: &SumTree<Slab>, offset: usize, slab: Slab) -> SumTree<Slab> {
    todo!()
}
...
impl sumtree::Summary for Metrics {
    fn combine(&self, other: &Self) -> Self {
        ...
        let num_lines = self.num_lines + other.num_lines;
        Metrics { len, num_lines }
    }
}
